...
try {
  ...
  if (Buffer.isBuffer(sp)) {
    equal(sp.toString().trim(), 'passed!', 'child process misconfigured 1')
  ...
  equal(status, 0, 'exit code should be 0 1')
  return
}
...
if (!sp.on) {
  ...
  // equal(sp.status, 0, 'exit code should be 0 2')
  // equal(stdout.toString().trim(), 'passed!', 'child process misconfigured 2')
  return
}
...
const timeout = setTimeout(() => {
  equal(out.toString().trim(), 'passed!', 'child process misconfigured 3')
}, 1000)

sp.once('exit', (status) => {
  equal(status, 0, 'exit code should be 0 3')
  equal(out.toString().trim(), 'passed!', 'child process misconfigured 4')
